...
public class Main {
    ...
    private static void printDetails() {
        ...
        for (Pair s : updated)
            System.out.println("<string name=\"" +s.getKey() + "\">" +  s.getValue() + "</string>");
        System.out.println("----------------------------");
    ...
    private static StringBuilder buildResult(String constData, List<Pair<String, String>> resultTranslated, List<Pair<String, String>> resultNotTranslated) {
        ...
        for (int i = 0; i < resultTranslated.size() ; i++) {
            builder.append(" <string name=\"" +resultTranslated.get(i).getKey() + "\">" +  resultTranslated.get(i).getValue() + "</string>" + "\n");
        }
        builder.append(constData);
        builder.append(NOT_TRANSLATED_TAG + "\n");
        for (int i = 0; i < resultNotTranslated.size() ; i++) {
            builder.append(" <string name=\"" +resultNotTranslated.get(i).getKey() + "\">" +  resultNotTranslated.get(i).getValue() + "</string>" + "\n");
        }
        builder.append("</resources>");
        return builder;
    }
    ...
    private static void writeDataToFile(StringBuilder builder) throws Exception {
        File file = new File("F:\\StringsMerger\\out2\\artifacts\\StringsMerger_jar\\" +"result" + ".xml");
        if (file.exists())
            file.delete();
        file.createNewFile();
    ...
    private static HashMap<String,String> getMap(String fileName, boolean translated) throws Exception {
        File file = new File("F:\\StringsMerger\\out2\\artifacts\\StringsMerger_jar\\" + fileName + ".xml");
        if (!file.exists())
    ...
    private static String readFile(String fileName, List<String> ignoreTags) throws IOException {
        BufferedReader br = new BufferedReader(new FileReader("F:\\StringsMerger\\out2\\artifacts\\StringsMerger_jar\\" +fileName + ".xml"));
        try {
